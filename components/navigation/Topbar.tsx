...
export function Topbar({ sidebarCollapsed, onOpenCommandBar }: TopbarProps) {
  const { profile, signOut } = useAuth();

  return (
    <header
      className={cn(
        'fixed top-0 right-0 z-30 flex h-14 items-center justify-between border-b border-border bg-bg-secondary/80 backdrop-blur-md px-6 transition-all duration-200',
    ...
    >
      ...
      >
        ...
        <kbd className="ml-4 hidden rounded border border-border bg-bg-elevated px-1.5 py-0.5 text-[10px] font-mono text-text-muted sm:inline">
          ⌘K
        </kbd>
      </button>
